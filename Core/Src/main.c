...
/**
 * @file main.c
 * @brief Caesar cipher cracking exercise.
...
void caesar(void) {
  ...
  while (str_len < MAX_STR_LEN) {
    ...
    if(HAL_UART_Receive(&huart2, (uint8_t *)&input_buffer[str_len], 1, HAL_MAX_DELAY) != HAL_OK) {
      printf("Error receiving character\r\n");
      return;
    }
    // printf("The character you sent is %c\r\n", input_buffer[str_len]);


    // TODO: If you received a '.' stop collecting input:
  if(input_buffer[str_len] == '.') {
    break;
  }

    // TODO: Otherwise, add the character you received to the input buffer, and
    //       increment str_len.
   str_len++;
  }

  // TODO: Zero-terminate the input string in the buffer:
  //       Make sure the last character of the string in you array is followed
  //       by a zero.
  ...
  for (unsigned int shift = 1; shift < 26; shift++) {
    ...
    for (unsigned int i = 0; i < str_len; i++) {
      ...
      // TODO: If the character is not a letter, put it into the output buffer as is and move to the
      //       next loop iteration to continue with the next character.
     if((character < 'A' || (character > 'Z' && character < 'a') || character > 'z')) { // uses ASCII values to check if the character is a letter
      ...
      unsigned int position_in_abc = 0;
      if(character >= 'A' && character <= 'Z'){
        position_in_abc = character - 'A';
      }else{
        position_in_abc= character - 'a'; 
      }
      ...
      unsigned int result_position_in_abc = (position_in_abc + shift) % 26; // why? Because we want to wrap around the alphabet, so if we go below 0 we want to start again at 25.
      char result_character = 0; // we will store the decoded character here

      if(character >= 'A' && character <= 'Z'){
        result_character = result_position_in_abc + 'A';
      }else{
        result_character = result_position_in_abc + 'a';
      }
